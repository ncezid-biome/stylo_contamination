...
outdir_prefix="kalamari-$VERSION"
...
TSV="$tempdir/in.tsv"
cat /opt/conda/envs/kalamari/src/chromosomes.tsv       > $TSV
tail -n +2 /opt/conda/envs/kalamari/src/plasmids.tsv  >> $TSV

# Include genomes that are not complete
if [[ ${KALAMARI_EXPERIMENTAL:-} ]]; then
  tail -n +2 /opt/conda/envs/kalamari/src/chromosomes-incomplete.tsv >> $TSV
fi

cp -rv /opt/conda/envs/kalamari/src/taxonomy $tempdir/taxonomy
